...
func pemBlockForKey(priv interface{}) *pem.Block {
	...
	case *ecdsa.PrivateKey:
		...
		//t1 := time.Now()
		//fmt.Printf("PEM KEY: The call took %v to run.\n", t1.Sub(t0))
		return x
	default:
		//t1 := time.Now()
		//fmt.Printf("PEM KEY: The call took %v to run.\n", t1.Sub(t0))
...
func create_connection(server, port, private_key string) *C.BIO {
	...
	//t1 := time.Now()
	//fmt.Printf("CONNECTION: The call took %v to run.\n", t1.Sub(t0))
...
func protocol_version_exchange(bio *C.BIO) int {
	...
	C.free(unsafe.Pointer(protocol_version))
	C.free(unsafe.Pointer(server_protocol_version))

	//t1 := time.Now()
	//fmt.Printf("VERSION EXCHANGE: The call took %v to run.\n", t1.Sub(t0))
...
func generate_key(bio *C.BIO, cert_type string) (interface{}, string) {
	...
	//t2 := time.Now()
	//fmt.Printf("GENERATE KEY: The call took %v to run. po zapisani SPOC\n", t2.Sub(t0))
	...
	//t3 := time.Now()
	//fmt.Printf("GENERATE KEY: The call took %v to run. po zapisani CA/END\n", t3.Sub(t2))
	...
	//t4 := time.Now()
	//fmt.Printf("GENERATE KEY: The call took %v to run. po zisteni key length \n", t4.Sub(t3))
	...
	//t5 := time.Now()	
	//fmt.Printf("GENERATE KEY: The call took %v to run. po vygenerovani key\n", t5.Sub(t4))

	C.free(unsafe.Pointer(sub_id))
	C.free(unsafe.Pointer(ca_info))
	C.free(unsafe.Pointer(key_length))

	//t1 := time.Now()
	//fmt.Printf("GENERATE KEY: The call took %v to run.\n", t1.Sub(t0))
...
func create_csr(bio *C.BIO, ip, dn string, key interface{}) string {
	...
	//t1 := time.Now()
	//fmt.Printf("CREATE CSR: The call took %v to run.\n", t1.Sub(t0))
...
func sending_csr(bio *C.BIO, ip, file string) (int, string, string) {
	...
	if !(strings.HasPrefix(str, "-----BEGIN")) {
		return -1, "error: server does not send correct certificate", ""
	}

	C.free(unsafe.Pointer(csr))
	C.free(unsafe.Pointer(cert_read))

	//t1 := time.Now()
	//fmt.Printf("SEND CSR: The call took %v to run.\n", t1.Sub(t0))
...
func save_cert(cert, path string) int {
	...
	t1 := time.Now()
	fmt.Printf("SAVE CERT: The call took %v to run.\n", t1.Sub(t0))
...
func save_key(key interface{}, path string) int {
	...
	t1 := time.Now()
	fmt.Printf("SAVE KEY: The call took %v to run.\n", t1.Sub(t0))
...
func main() {
	...
	arg := os.Args // arguments: file, server, port, key, path, subjectIp, subjectDN, subjectID
	if len(arg) < 8 {
		log.Fatalf("Too few arguments. [server, port, key, path, subjectIp, subjectDN, subjectID]")
	} else if len(arg) > 8 {
		log.Fatalf("Too many arguments.[server, port, key, path, subjectIp, subjectDN, subjectID]")
	}
	...
	if bio == nil {
		log.Fatalf("error while connecting to server, check psk_key")
	}

	//2. protocol version exchange
	if protocol_version_exchange(bio) == -1 {
	...
	//5. achieving trust
	something_to_read := C.C_bio_read(bio)
	str := C.GoString(something_to_read)

	if !(strings.HasPrefix(str, "-----BEGIN")) {
		log.Fatalf("error: server does not send correct trust anchor")
	} else {
		//fmt.Println("server sends correct trust anchor")
	}
